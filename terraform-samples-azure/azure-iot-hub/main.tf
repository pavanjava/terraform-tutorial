resource "random_pet" "prefix_rg" {
  prefix = var.resource_group_name_prefix
}

resource "azurerm_resource_group" "iot_rg" {
  location = var.resource_group_location
  name     = random_pet.prefix_rg.id
}

# Create storage account & container
resource "random_string" "iot_sa_name" {
  length  = 12
  special = false
  upper   = false
}

resource "azurerm_storage_account" "iot_sa" {
  name                     = random_string.iot_sa_name.id
...
resource "azurerm_storage_container" "my_terraform_container" {
  name                  = "iotcontainer"
  storage_account_name  = azurerm_storage_account.iot_sa.name
...
resource "azurerm_eventhub_authorization_rule" "iot_terraform_authorization_rule" {
  resource_group_name = azurerm_resource_group.iot_rg.name
  namespace_name      = azurerm_eventhub_namespace.namespace.name
...
resource "azurerm_iothub" "iothub" {
  ...
  endpoint {
    ...
    connection_string          = azurerm_storage_account.iot_sa.primary_blob_connection_string
    name                       = "export"
  ...
  endpoint {
    ...
    connection_string = azurerm_eventhub_authorization_rule.iot_terraform_authorization_rule.primary_connection_string
    name              = "export2"
  }
